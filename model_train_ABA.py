...
if NetworkName == "ABA-FWI":
    net = ABA_FWI(n_classes=OutChannel,
                     in_channels=InChannel,
                     is_deconv=True,
                     is_batchnorm=True)

elif NetworkName == "ABA-FWI+":
    net = ABA_FWI_SEG(n_classes=OutChannel,
                       in_channels=InChannel,
                       is_deconv=True,
                       is_batchnorm=True)
...
def train():
    ...
    for i, (seismic_datas, vmodels, edges) in enumerate(train_loader):
        ...
        outputs = net(seismic_datas, ModelDim)
...
def validate():
    ...
    with torch.no_grad():
        for i, (seismic_datas, vmodels, edges, vmodel_max_min) in enumerate(val_loader):
            seismic_datas = seismic_datas[0].to(device)
            vmodels = vmodels[0].to(device)
            edges = edges[0].to(device)
            outputs = net(seismic_datas, ModelDim)
