...
def apply_gaussian_blur(para_image, kernel_size=15, sigma=0):
    '''
    Apply Gaussian blur to the input image.

    :param para_image: Input image (numpy array).
    :param kernel_size: Size of the Gaussian kernel (must be positive and odd).
    :param sigma: Standard deviation of the Gaussian kernel. If it is 0, it is calculated from the kernel size.
    :return: Blurred image (same data type and range as the input image, but blurred).
    '''
    blurred_image= cv2.GaussianBlur(para_image, (kernel_size, kernel_size), sigma)
...
def SaveLearningRate(learning_rates, SavePath, ModelName):
    ...
    plt.plot(range(0, Epochs), learning_rates, label='Learning Rate')
